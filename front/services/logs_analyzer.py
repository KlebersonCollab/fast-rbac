...
class LogAnalyzer:
    ...
    def read_log_file(self, category: str, filename: str, 
    ...
    def get_logs_summary(self, time_range_hours: int = 24) -> Dict[str, Any]:
        ...
        for entry in all_entries:
            try:
                ...
                if 'timestamp' in entry:
                    ...
                    entry_time = datetime.fromisoformat(timestamp_str.replace('Z', '+00:00').replace('+00:00', ''))
        ...
        if response_times:
            summary["performance_stats"]["avg_response_time"] = sum(response_times) / len(response_times)
        
        # Ordena e limita resultados
        summary["recent_errors"] = sorted(
            summary["recent_errors"][-20:], 
    ...
    def get_filtered_logs(self, category: str = None, log_file: str = None,
                         level: str = None, username: str = None,
                         search_term: str = None, time_range_hours: int = 24,
                         max_entries: int = 500) -> List[Dict[str, Any]]:
    ...
    def get_performance_metrics(self, time_range_hours: int = 24) -> Dict[str, Any]:
            ...
            try:
                if 'timestamp' in entry:
                    ...
                    if isinstance(duration, (int, float)) and duration > 0:
                        metrics["avg_response_time_by_endpoint"][endpoint].append(duration)
        ...
        for endpoint, times in metrics["avg_response_time_by_endpoint"].items():
            if times:
                metrics["avg_response_time_by_endpoint"][endpoint] = sum(times) / len(times)
        
        # Ordena requests lentos
        metrics["slow_requests"] = sorted(
            metrics["slow_requests"][-50:], 
    ...
    def get_user_activity_stats(self, time_range_hours: int = 24) -> Dict[str, Any]:
        ...
        user_entries = self.read_log_file("Frontend", "user_actions/actions.log", max_lines=3000)
        ...
        perm_entries = self.read_log_file("Frontend", "permissions/permissions.log", max_lines=2000)
