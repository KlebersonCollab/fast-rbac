...
def render_posts_page():
    ...
    st.markdown(
        "Esta é uma página de exemplo para demonstrar o controle de acesso baseado em permissões."
    )
...
def render_create_post():
    ...
    with st.form("create_post_form"):
        ...
        content = st.text_area(
            "Conteúdo", placeholder="Escreva o conteúdo do post...", height=200
        )
...
def render_settings_page():
    ...
    with st.expander("📧 Configurações de Email"):
        if auth_service.has_permission("settings:update"):
            st.text_input(
                "Servidor SMTP", placeholder="smtp.exemplo.com", disabled=True
            )
