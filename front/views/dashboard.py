...
def render_dashboard():
    ...
    try:
        ...
        users_data = (
            api_client.get_users() if auth_service.has_permission("users:read") else []
        )
        roles_data = (
...
def render_metrics_cards(users_data, roles_data, permissions_data):
    ...
    with col2:
        ...
        st.metric(label="🎭 Papéis", value=total_roles, delta=f"{active_roles} ativos")

    with col3:
        total_permissions = len(permissions_data)

        st.metric(label="🔐 Permissões", value=total_permissions, delta="Sistema RBAC")
    ...
    with col4:
        ...
        st.metric(
            label="🌐 OAuth Users", value=oauth_users, delta=f"{oauth_percentage:.1f}%"
        )
...
def render_roles_chart(roles_data):
    ...
    for role in roles_data:
        ...
        mock_counts = {"admin": 2, "manager": 5, "editor": 12, "viewer": 25}
        ...
        **📝 Como testar:**
        ...
        if st.button("🧪 Testar Verificação de Permissão", use_container_width=True):
            test_permission = st.text_input(
                "Permissão para testar:", placeholder="ex: posts:create"
            )
            if test_permission:
