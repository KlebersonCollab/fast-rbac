...
def render_user_card(user, roles_data, index):
    ...
    with col2:
        ...
        is_superadmin = user.get('is_superuser', False) or any(r.get('name') == 'superadmin' for r in roles)
    ...
    with col4:
        ...
        current_is_superadmin = current_user.get('is_superuser', False) or any(r.get('name') == 'superadmin' for r in current_user.get('roles', []))
        target_is_superadmin = user.get('is_superuser', False) or any(r.get('name') == 'superadmin' for r in user.get('roles', []))
        
        # Botão editar com proteção hierárquica
        can_edit = auth_service.has_permission("users:update") and (current_is_superadmin or not target_is_superadmin)
        ...
        can_delete = (auth_service.has_permission("users:delete") and 
                     user.get('username') != 'admin' and  # Protect admin user
                     user.get('id') != current_user.get('id') and  # Can't delete yourself
                     (current_is_superadmin or not target_is_superadmin))  # Hierarchy protection
...
def render_edit_user_form(user, roles_data):
    ...
    current_user = auth_service.get_current_user()
    is_superadmin = current_user.get('is_superuser', False) or any(r.get('name') == 'superadmin' for r in current_user.get('roles', []))
    target_is_superadmin = user.get('is_superuser', False) or any(r.get('name') == 'superadmin' for r in user.get('roles', []))
    user_id = user.get('id')
    
    # Mostrar status de superadmin
    if user.get('is_superuser', False) or any(r.get('name') == 'superadmin' for r in user.get('roles', [])):
    ...
    if is_superadmin and user_id != current_user.get('id'):
        ...
        with col_promote:
            if not target_is_superadmin and st.button(f"⬆️ Promover a Superadmin", key=f"promote_{user_id}"):
                try:
                    response = api_client.post(f"/admin/users/{user_id}/superadmin")
                    st.success(f"Usuário {user.get('username')} promovido a superadmin!")
                    st.rerun()
                except Exception as e:
                    st.error(f"Erro ao promover: {str(e)}")
        
        with col_demote:
            if target_is_superadmin and st.button(f"⬇️ Revogar Superadmin", key=f"demote_{user_id}"):
                try:
                    response = api_client.delete(f"/admin/users/{user_id}/superadmin")
                    st.success(f"Privilégios de superadmin revogados de {user.get('username')}!")
                    st.rerun()
                except Exception as e:
        ...
        if roles_data and auth_service.has_permission("users:update"):
            ...
            for role in available_roles:
                ...
                with col2:
                    ...
                    disabled = (role_name == 'superadmin' and not is_superadmin) or (not is_superadmin and target_is_superadmin)
