...
from front.utils.helpers import confirm_action, format_datetime, format_permission_badge
...
def render_roles_page():
    ...
    try:
        ...
        tab1, tab2, tab3 = st.tabs(
            ["📋 Lista de Papéis", "➕ Novo Papel", "📊 Estatísticas"]
        )
...
def render_roles_list(roles_data, permissions_data):
    ...
    search_term = st.text_input(
        "🔍 Buscar papéis", placeholder="Digite o nome ou descrição..."
    )
    ...
    if search_term:
        filtered_roles = [
            ...
            if (
                search_term.lower() in role.get("name", "").lower()
                or search_term.lower() in role.get("description", "").lower()
            )
        ]
    ...
    with st.form(f"edit_role_form_{role.get('id')}"):
        ...
        with col1:
            ...
            new_is_active = st.checkbox(
                "Papel Ativo", value=role.get("is_active", False)
            )

        with col2:
            new_description = st.text_area(
                "Descrição", value=role.get("description", "")
            )
        ...
        current_permission_ids = [
            perm.get("id") for perm in role.get("permissions", [])
        ]
        role_id = role.get("id")
                        ...
                        with col2:
                            ...
                            if new_has_permission != has_permission:
                                if new_has_permission and not has_permission:
                                    # Assign permission
                                    try:
                                        api_client.assign_permission_to_role(
                                            role_id, perm_id
                                        )
                                        ...
                                        st.error(
                                            f"Erro ao atribuir permissão: {e.message}"
                                        )
                                elif has_permission and not new_has_permission:
                                    # Remove permission
                                    try:
                                        api_client.remove_permission_from_role(
                                            role_id, perm_id
                                        )
                                        ...
                                        st.error(
                                            f"Erro ao remover permissão: {e.message}"
                                        )
    ...
    with st.form("create_role_form"):
        ...
        with col2:
            description = st.text_area(
                "Descrição", placeholder="Descrição do papel e suas responsabilidades"
            )
...
def render_roles_statistics(roles_data, permissions_data):
    ...
    with col2:
        ...
        if permissions_data:
            ...
            if perm_usage:
                # Sort by usage count
                sorted_perms = sorted(
                    perm_usage.items(), key=lambda x: x[1], reverse=True
                )[:10]

                df_perm_usage = pd.DataFrame(
                    [{"Permissão": perm, "Usos": count} for perm, count in sorted_perms]
                )
    ...
    sorted_roles = sorted(
        roles_data, key=lambda x: len(x.get("permissions", [])), reverse=True
    )
        ...
        try:
            ...
            for role in roles_data:
                permissions_str = ", ".join(
                    [perm.get("name", "") for perm in role.get("permissions", [])]
                )
                export_data.append(
                    {
                        "Papel": role.get("name", ""),
