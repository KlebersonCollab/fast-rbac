...
def render_permissions_page():
    ...
    try:
        ...
        tab1, tab2, tab3 = st.tabs(
            ["📋 Lista de Permissões", "➕ Nova Permissão", "📊 Estatísticas"]
        )
...
def render_permissions_list(permissions_data, roles_data):
    ...
    with col1:
        search_term = st.text_input(
            "🔍 Buscar permissões", placeholder="Digite o nome, recurso ou ação..."
        )

    with col2:
        # Resource filter
        resources = list(
            set([perm.get("resource", "other") for perm in permissions_data])
        )
    ...
    if search_term:
        filtered_permissions = [
            ...
            if (
                search_term.lower() in perm.get("name", "").lower()
                or search_term.lower() in perm.get("resource", "").lower()
                or search_term.lower() in perm.get("action", "").lower()
                or search_term.lower() in perm.get("description", "").lower()
            )
        ]
    ...
    st.info(
        f"Encontradas {len(filtered_permissions)} permissão(ões) de {len(permissions_data)} total."
    )
    ...
    for resource, perms in sorted(permissions_by_resource.items()):
        with st.expander(
            f"📁 {resource.title()} ({len(perms)} permissões)", expanded=True
        ):
                ...
                if st.button("🗑️", key=f"delete_perm_{index}", help="Deletar permissão"):
                    if st.session_state.get(f"confirm_delete_perm_{index}", False):
                        try:
                            ...
                            st.warning(
                                "Funcionalidade de deletar permissão não implementada no backend."
                            )
    ...
    with st.form("create_permission_form"):
        ...
        with col1:
            ...
            if selected_resource == "outros":
                custom_resource = st.text_input(
                    "Recurso personalizado", placeholder="nome_do_recurso"
                )
        ...
        with col2:
            ...
            if selected_action == "outros":
                custom_action = st.text_input(
                    "Ação personalizada", placeholder="nome_da_acao"
                )
        ...
        name = st.text_input(
            "Nome da Permissão *", value=suggested_name, placeholder="recurso:acao"
        )
        description = st.text_area(
            "Descrição", placeholder="Descreva o que esta permissão permite fazer"
        )
        ...
        if st.form_submit_button("✅ Criar Permissão", use_container_width=True):
            ...
            if not all([name, resource, action]):
                ...
                try:
                    response = api_client.create_permission(
...
def render_permissions_statistics(permissions_data, roles_data):
    ...
    with col3:
        # Unique actions
        unique_actions = len(
            set(perm.get("action", "other") for perm in permissions_data)
        )
    ...
    with col1:
        ...
        if used_perms:
            sorted_used = sorted(used_perms.items(), key=lambda x: x[1], reverse=True)[
                :10
            ]
    ...
    with col2:
        ...
        if unused_perms:
            st.warning(
                f"{len(unused_perms)} permissão(ões) não estão sendo utilizadas:"
            )
