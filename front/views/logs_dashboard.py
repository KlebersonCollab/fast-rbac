...
def convert_timestamp_with_timezone(timestamp_series, format_str="%Y-%m-%d %H:%M:%S"):
    """Convert timestamp series to specified timezone and format"""
    try:
        # Parse ISO8601 timestamps
        dt_series = pd.to_datetime(timestamp_series, format="ISO8601")
        ...
        # Convert to configured timezone
        dt_series = dt_series.dt.tz_convert(settings.timezone)

        # Format to string
        return dt_series.dt.strftime(format_str)
...
def show_logs_dashboard():
    """Dashboard para monitoramento de logs em tempo real"""
    import time
    ...
    st.set_page_config(page_title="Dashboard de Logs", page_icon="📊", layout="wide")

    # Verificação já feita na sidebar - acesso autorizado
            ...
            with alert_cols[i % 3]:
                ...
                with st.container():
                    if container_type == "error":
                        st.error(
                            f"{alert_color} **{alert['type'].title()}**\n\n{alert['message']}"
                        )
    ...
    with metric_cols[1]:
        ...
        st.metric(label="❌ Erros", value=error_count, delta=None)
    ...
    with metric_cols[3]:
        ...
        st.metric(label="🐌 Requests Lentos", value=slow_requests, delta=None)
    ...
    with tab1:
        ...
        with col2:
            ...
            if summary["by_category"]:
                ...
                fig_categories.update_layout(
                    xaxis_title="Categoria", yaxis_title="Número de Logs", height=400
                )
        ...
        if summary["by_hour"]:
            ...
            fig_timeline = px.line(
                x=hours, y=counts, title="Atividade por hora", markers=True
            )

            fig_timeline.update_layout(
                xaxis_title="Hora", yaxis_title="Número de Logs", height=300
            )
    ...
    with tab2:
        ...
        with filter_col3:
            ...
            search_term = st.text_input(
                "🔍 Buscar:", placeholder="Digite para filtrar..."
            )
        ...
        if filtered_logs:
            ...
            if available_columns:
                ...
                if "timestamp" in display_df.columns:
                    display_df["timestamp"] = convert_timestamp_with_timezone(
                ...
                if "message" in display_df.columns:
                    display_df["message"] = (
                        display_df["message"].astype(str).str[:100] + "..."
                    )
                ...
                st.warning(
                    "Não foi possível exibir os logs - colunas esperadas não encontradas."
                )
        else:
    ...
    with tab3:
        ...
        with user_col1:
            st.metric("👥 Usuários Ativos", len(user_stats.get("active_users", [])))
        ...
        if user_stats.get("page_views"):
            ...
            fig_pages = px.pie(
                values=views, names=pages, title="Distribuição de page views"
            )
    ...
    with tab4:
        ...
        if perf_metrics.get("slow_requests"):
            ...
            if slow_requests:
                ...
                if "timestamp" in slow_df.columns:
                    slow_df["timestamp"] = convert_timestamp_with_timezone(
                        slow_df["timestamp"]
                    )
        ...
        if perf_metrics.get("avg_response_time_by_endpoint"):
            ...
            numeric_times = {
                k: v for k, v in response_times.items() if isinstance(v, (int, float))
            }
    ...
    if summary.get("recent_errors"):
        ...
        if not errors_df.empty:
            ...
            if "timestamp" in errors_df.columns:
                errors_df["timestamp"] = convert_timestamp_with_timezone(
                    errors_df["timestamp"]
                )
            ...
            if "message" in errors_df.columns:
                errors_df["message"] = (
                    errors_df["message"].astype(str).str[:150] + "..."
                )

            # Seleciona apenas colunas que existem
            desired_columns = ["timestamp", "level", "category", "logger", "message"]
            available_columns = [
                col for col in desired_columns if col in errors_df.columns
            ]

            if available_columns:
                st.dataframe(
                    errors_df[available_columns], use_container_width=True, height=200
                )
            else:
