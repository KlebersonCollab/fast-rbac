...
def show_oauth_providers():
    ...
    try:
        ...
        if providers:
            st.info(
                "Os provedores OAuth estão configurados, mas o login social requer configuração adicional no navegador."
            )
            ...
            for provider in providers:
                ...
                # Provider icons
                icon_map = {"google": "🟡", "microsoft": "🔵", "github": "⚫"}
                icon = icon_map.get(name, "🔗")

                if st.button(
                    f"{icon} Entrar com {display_name}",
