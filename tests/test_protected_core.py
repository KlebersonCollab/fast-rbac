...
class TestProtectedCore:
    ...
    def test_access_settings_authenticated(self, client: TestClient, admin_auth_headers):
...
class TestProtectedPermissions:
    ...
    def test_protected_endpoints_require_authentication(self, client: TestClient):
        ...
        for endpoint in protected_endpoints_get:
            ...
            assert response.status_code == status.HTTP_403_FORBIDDEN, f"Endpoint {endpoint} should require authentication"
        
        # Teste endpoint POST separadamente
        response = client.post("/protected/posts/create")
        assert response.status_code == status.HTTP_403_FORBIDDEN, "POST endpoint should require authentication"
    
    def test_protected_endpoints_work_with_valid_token(self, client: TestClient, admin_auth_headers):
        ...
        for endpoint in protected_endpoints_get:
            ...
            assert response.status_code == status.HTTP_200_OK, f"Endpoint {endpoint} should work with valid token"
        
        # Teste para endpoint POST
        response = client.post("/protected/posts/create", headers=admin_auth_headers)
        assert response.status_code == status.HTTP_200_OK, "POST endpoint should work with valid token" 