...
class CacheService:
    ...
    async def increment_rate_limit(self, key: str, window: int = 60) -> int:
        ...
        try:
            ...
            if count == 1:
                ...
                await asyncio.to_thread(redis_service.redis_client.expire, cache_key, window)
            return count
    ...
    async def set_query_result(self, query_hash: str, result: Any, ttl: Optional[int] = None) -> bool:
        """Cache query result"""
        cache_key = self._get_cache_key("query", query_hash)
            ...
            return {
                ...
                "application_cache": {
                    ...
                    "key_patterns": {
                        ...
                        "permissions": len([k for k in our_keys if ":permissions:" in k]),
                        "roles": len([k for k in our_keys if ":roles:" in k]),
                        "sessions": len([k for k in our_keys if ":session:" in k]),
                        "rate_limits": len([k for k in our_keys if ":rate_limit:" in k]),
