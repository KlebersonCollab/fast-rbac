...
from app.models.schemas import TotpSetupResponse, TotpVerifyResponse, TotpStatusResponse
...
class TotpService:
    ...
    def verify_totp(self, user: User, totp_code: str, window: int = 1) -> TotpVerifyResponse:
        """
        ...
        if not user.totp_secret:
            return TotpVerifyResponse(
                success=False,
                message="2FA não está configurado para este usuário"
            )
        ...
        try:
            ...
            if user.last_totp_used == totp_code:
                return TotpVerifyResponse(
                    success=False,
                    message="Código TOTP já foi usado"
                )
            ...
            return TotpVerifyResponse(
                success=False,
                message=f"Erro na verificação TOTP: {str(e)}"
            )
    
    def verify_backup_code(self, user: User, backup_code: str, db: Session) -> TotpVerifyResponse:
        """
    ...
    def disable_2fa(self, user: User, password: str, totp_code: str = None, 
                    backup_code: str = None, db: Session = None) -> TotpVerifyResponse:
        """
        ...
        if totp_code:
            ...
            return TotpVerifyResponse(
                success=False,
                message="Código TOTP ou de backup é necessário"
            )
    ...
    def regenerate_backup_codes(self, user: User, totp_code: str, db: Session) -> TotpVerifyResponse:
        """
    ...
    def authenticate_with_2fa(self, username: str, password: str, 
                             totp_code: str = None, backup_code: str = None, 
                             db: Session = None) -> Tuple[bool, str, Optional[User]]:
        """
