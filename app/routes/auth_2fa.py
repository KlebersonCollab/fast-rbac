...
async def disable_2fa(
    ...
    result = totp_service.disable_2fa(
        current_user, request.password, request.totp_code, request.backup_code, db
    )
...
async def login_with_2fa(request: LoginWith2FARequest, db: Session = Depends(get_db)):
    """
    Login com suporte a 2FA
    """
    success, message, user = totp_service.authenticate_with_2fa(
        request.username, request.password, request.totp_code, request.backup_code, db
    )
...
async def get_qr_code(
    ...
    try:
        ...
        provisioning_uri = totp.provisioning_uri(
            name=current_user.email, issuer_name=totp_service.issuer
        )
...
async def test_backup_code(
    ...
    try:
        ...
        if is_valid:
            return TotpVerifyResponse(success=True, message="Código de backup válido")
        else:
