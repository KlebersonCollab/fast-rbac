...
class RateLimitingMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        response.headers["X-RateLimit-Limit"] = str(
            self.rate_limits[rate_limit_type]["requests"]
        )
    ...
    async def _check_rate_limit(
        self, client_id: str, rate_limit_type: str, path: str
    ...
    async def _rate_limit_exceeded_response(
        self, remaining: int, reset_time: float
    ) -> JSONResponse:
...
async def check_rate_limit(
    client_id: str, rate_limit_type: str = "default"
) -> Tuple[bool, int, float]:
