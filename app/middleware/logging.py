...
from app.config.logging import get_api_logger, get_error_logger, log_api_request, log_error
...
class HealthCheckLoggingMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app: ASGIApp, health_endpoints: list = None):
        super().__init__(app)
        self.health_endpoints = health_endpoints or ["/health", "/health/", "/healthcheck"]
...
class UserActivityLoggingMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        should_track = any(
            ...
            for endpoint in self.track_endpoints
        )
